 ...
 *
 * Controls at the edges of the card list:
 * - **−** (start) — remove all combatants (requires a confirmation dialog).
 * - **+** (end)   — append a new blank combatant card.
 *
...
class TrackerPlugin : DmPlugin {
    ...
    /**
     * Builds the full card pane for the given [orientation].
     *
    ...
    private fun buildCardPane(orientation: Orientation, refresh: () -> Unit): Pane {
        ...
        // "−" Remove-all button at the start of the order.
        ...
        container.children.add(removeAllBtn)
