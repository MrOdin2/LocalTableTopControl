...
- Exported walls carry a wall kind. Soft walls match the original wall behavior: they block PC
  ...
  also block dim light. Feature walls are directional: the exported normal arrow defines the front
  side, and the front/back side behaviors can be Open, Soft, or Hard. Open sides are transparent
  from that side; Soft sides block sightlines and bright light but allow dim light; Hard sides block
  sightlines, bright light, and dim light.
- When authored or PC-carried lighting is active, the shared sightline service builds a light mask
  ...
  their dim radius as a soft reach through soft walls and Soft feature-wall sides, while their bright radius is occluded by all
  closed exported walls and blocking feature-wall sides. Hard walls, closed doors, and Hard feature-wall sides occlude both bright and dim light, with dim light using the larger dim range.
...
- Tracker light-source ranges are published as token metadata in grid cells. DynamicMap consumes
  ...
  soft walls and Soft feature-wall sides but is clipped by hard walls and Hard feature-wall sides, and the bright radius reuses the cached PC sight
...
- Current lighting supports builder-authored point lights, sunlight/outside polygons, and PC-carried
  ...
  approximation and is blocked by hard walls, closed doors, and Hard feature-wall sides when authored. Light colour tint is rendered for visible point-light areas. PC darkvision is supported
