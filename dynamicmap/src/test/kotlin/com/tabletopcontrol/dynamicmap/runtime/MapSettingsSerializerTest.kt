...
class MapSettingsSerializerTest {
    ...
    fun `serialized text contains expected keys`() {
        ...
        assert(text.contains("dynamicMap.renderMode=")) { "Missing dynamicMap.renderMode" }
    }
    ...
    @Test
    fun `deserializeDynamicMapRenderMode round-trips debug mode`() {
        ...
        assertEquals(DynamicMapRenderMode.DEBUG, MapSettingsSerializer.deserializeDynamicMapRenderMode(text))
    }

    @Test
    fun `deserializeDynamicMapRenderMode returns null when key absent`() {
        val text = "map.scale=1.0\nmap.offsetX=0.0\nmap.offsetY=0.0"

        assertNull(MapSettingsSerializer.deserializeDynamicMapRenderMode(text))
    }
    ...
    fun `deserializeAll round-trips all six settings including rotation and table offset`() {
        ...
        assertEquals(DynamicMapRenderMode.RENDER, settings.dynamicMapRenderMode)
    }
    ...
    fun `deserializeAll returns null mapRotation for old config without rotation key (backward compatibility)`() {
        ...
        assertNull(settings.dynamicMapRenderMode)
    }
}
