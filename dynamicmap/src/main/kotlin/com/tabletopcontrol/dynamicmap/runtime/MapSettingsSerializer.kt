 ...
 * @property dynamicMapRenderMode saved DynamicMap visibility mode, or `null` if absent or unparseable.
 */
data class MapSavedSettings(
    ...
    val dynamicMapRenderMode: DynamicMapRenderMode? = null,
)
...
object MapSettingsSerializer {
    ...
    internal const val CONFIG_NAME = "dynamicmap-settings.conf"
     ...
     *
     * @param gridColor       grid line colour to persist; defaults to [com.tabletopcontrol.dynamicmap.runtime.logic.GridConfig.color].
     ...
     * @param dynamicMapRenderMode visibility/debug mode for DynamicMap wall and light data.
     */
    fun serialize(
        ...
        dynamicMapRenderMode: DynamicMapRenderMode = DynamicMapRenderMode.RENDER,
    ): String {
        ...
        sb.appendLine("dynamicMap.renderMode=${dynamicMapRenderMode.name}")
    ...
    /**
     * Parses the DynamicMap wall/light visibility mode from a properties-format [text].
     *
     * @return The parsed mode, or `null` if the key is absent or invalid.
     */
    fun deserializeDynamicMapRenderMode(text: String): DynamicMapRenderMode? {
        return try {
            val props = Properties()
            props.load(text.reader())
    ...
    fun deserializeAll(text: String): MapSavedSettings = MapSavedSettings(
        ...
        dynamicMapRenderMode = deserializeDynamicMapRenderMode(text),
    )
