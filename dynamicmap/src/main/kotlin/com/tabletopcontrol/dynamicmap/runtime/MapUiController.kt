...
private const val MAP_LOAD_HISTORY_KEY = "dynamicmap.load"
...
class MapUiController {
    ...
    fun captureSceneState(): String =
        MapSceneCodec.serialize(
            MapSceneState(
                dynamicMapBundlePath = settingsService.currentDynamicMapBundlePath,
                dynamicMapDisplayPath = settingsService.currentDynamicMapDisplayPath,
    ...
    fun applySceneState(payload: String) {
        ...
        if (state.fog == null) {
            settingsService.currentDynamicMap?.let { fogOfWarService.configureCoverage(it.cols, it.rows) }
        }
    ...
    private fun buildCompactControls(): VBox {
        val pathField = TextField(settingsService.currentDynamicMapDisplayPath.orEmpty()).apply {
            isEditable = false
            promptText = "No dynamic map loaded"
            tooltip = Tooltip("Path to the currently loaded Dynamic Map bundle")
        }
        HBox.setHgrow(pathField, Priority.ALWAYS)

        val loadButton = Button("Load Dynamic Map...").apply {
            tooltip = Tooltip("Open a Dynamic Map bundle exported by the builder")
        }

        val removeMapButton = Button("Clear Dynamic Map").apply {
            tooltip = Tooltip("Remove the current Dynamic Map bundle and return to a plain background colour")
        }

        val renderModeToggleGroup = ToggleGroup()
        val renderModeButton = ToggleButton(DynamicMapRenderMode.RENDER.displayName).apply {
            toggleGroup = renderModeToggleGroup
            tooltip = Tooltip("Hide builder walls and lights during normal play")
        }
        val debugModeButton = ToggleButton(DynamicMapRenderMode.DEBUG.displayName).apply {
            toggleGroup = renderModeToggleGroup
            tooltip = Tooltip("Show builder walls and two-radius light halos for setup checks")
        }
        fun syncRenderModeButtons() {
            when (settingsService.dynamicMapRenderMode) {
                DynamicMapRenderMode.RENDER -> renderModeButton.isSelected = true
                DynamicMapRenderMode.DEBUG -> debugModeButton.isSelected = true
            }
        }
        renderModeToggleGroup.selectedToggleProperty().addListener { _, oldToggle, newToggle ->
            if (newToggle == null) {
                oldToggle?.isSelected = true
                return@addListener
            }
            val selectedMode = if (newToggle === debugModeButton) {
                DynamicMapRenderMode.DEBUG
            } else {
                DynamicMapRenderMode.RENDER
            }
            settingsService.setDynamicMapRenderMode(selectedMode)
        }
        ...
        fun refreshLoadedMapState() {
            val hasLoadedMap = settingsService.currentDynamicMap != null
        ...
        loadButton.setOnAction { event ->
            val chooser = FileChooser().apply {
                title = "Select Dynamic Map bundle"
                extensionFilters.addAll(
                    FileChooser.ExtensionFilter("Dynamic Map bundles", "*.dynamicmap", "*.zip"),
                    FileChooser.ExtensionFilter("All files", "*.*"),
                )
                FileChooserHistoryStore.configureInitialDirectory(
                    chooser = this,
                    key = MAP_LOAD_HISTORY_KEY,
                    fallbackSelection = settingsService.currentDynamicMapDisplayPath?.let(::File),
                )
            }
            ...
            if (file != null) {
                ...
                settingsService.applyDynamicMapLoad(file)
                    .onSuccess {
                        fogOfWarService.configureCoverage(it.cols, it.rows)
                        refreshLoadedMapState()
                    }
                    .onFailure { error ->
                        Alert(Alert.AlertType.ERROR).apply {
                            title = "Load Dynamic Map"
                            headerText = "The Dynamic Map could not be loaded."
                            contentText = error.toUserMessage()
