...
import com.tabletopcontrol.dynamicmap.runtime.DynamicMapBundleLoader
...
import com.tabletopcontrol.dynamicmap.runtime.DynamicMapRenderModeEvent
...
import com.tabletopcontrol.dynamicmap.runtime.MapBackgroundEvent
import com.tabletopcontrol.dynamicmap.runtime.MapCalibrationEvent
import com.tabletopcontrol.dynamicmap.runtime.MapClearEvent
import com.tabletopcontrol.dynamicmap.runtime.MapCalibrationModeEvent
import com.tabletopcontrol.dynamicmap.runtime.MapInputField
import com.tabletopcontrol.dynamicmap.runtime.MapFogSceneMode
import com.tabletopcontrol.dynamicmap.runtime.MapFogSceneState
...
import com.tabletopcontrol.dynamicmap.runtime.MapRotationEvent
import com.tabletopcontrol.dynamicmap.runtime.MapSceneState
import com.tabletopcontrol.dynamicmap.runtime.MapSavedSettings
import com.tabletopcontrol.dynamicmap.runtime.MapSettingsSerializer
import com.tabletopcontrol.dynamicmap.runtime.MeasurementAddedEvent
import com.tabletopcontrol.dynamicmap.runtime.MeasurementOverlay
import com.tabletopcontrol.dynamicmap.runtime.MeasurementRemovedEvent
import com.tabletopcontrol.dynamicmap.runtime.MeasurementType
import com.tabletopcontrol.dynamicmap.runtime.MeasurementUpdatedEvent
import com.tabletopcontrol.dynamicmap.runtime.MeasurementsClearedEvent
import com.tabletopcontrol.dynamicmap.runtime.ShowTokenNamesEvent
import com.tabletopcontrol.dynamicmap.runtime.TableMapOffsetEvent
...
class MapSettingsService(savedSettings: MapSavedSettings = MapSettingsSerializer.load()) {
    ...
    var dynamicMapRenderMode: DynamicMapRenderMode =
        savedSettings.dynamicMapRenderMode ?: DynamicMapRenderMode.RENDER
        private set
    ...
    var currentDynamicMap: DynamicMapBundle? = null
        private set

    var currentDynamicMapBundlePath: String? = null
        private set

    var currentDynamicMapDisplayPath: String? = null
        private set
    ...
    internal fun applySceneState(sceneState: MapSceneState) {
        ...
        currentDynamicMapBundlePath = sceneState.dynamicMapBundlePath
        currentDynamicMapDisplayPath = sceneState.dynamicMapDisplayPath ?: sceneState.dynamicMapBundlePath
        ...
        if (dynamicMapPath != null) {
            when (val result = DynamicMapBundleLoader.load(File(dynamicMapPath))) {
                is MapResult.Success -> {
                    currentDynamicMap = result.value
                    currentDynamicMapBundlePath = result.value.sourcePath
                    currentDynamicMapDisplayPath = result.value.displayPath
                    EventBus.publish(DynamicMapLoadEvent(result.value))
                }
                is MapResult.Failure -> EventBus.publish(MapClearEvent)
            }
        } else if (currentMapImageUri != null) {
    ...
    fun applyMapLoad(uri: String, displayPath: String = uri) {
        currentDynamicMap = null
        currentDynamicMapBundlePath = null
        currentDynamicMapDisplayPath = null
        currentMapImageUri = uri
        currentMapDisplayPath = displayPath
        EventBus.publish(MapLoadEvent(uri))
    }

    fun applyDynamicMapLoad(file: File): MapResult<DynamicMapBundle> {
        val result = DynamicMapBundleLoader.load(file)
        if (result is MapResult.Success) {
            currentDynamicMap = result.value
            currentDynamicMapBundlePath = result.value.sourcePath
            currentDynamicMapDisplayPath = result.value.displayPath
            currentMapImageUri = null
            currentMapDisplayPath = null
            gridCalibration = centeredGridCalibration(result.value)
            currentGridConfig = GridConfig(color = gridColor)
            mapRotation = 0
            EventBus.publish(DynamicMapLoadEvent(result.value))
            EventBus.publish(GridCalibrationEvent(gridCalibration))
            EventBus.publish(GridUpdateEvent(currentGridConfig))
    ...
    fun setDynamicMapRenderMode(mode: DynamicMapRenderMode) {
        if (dynamicMapRenderMode == mode) return
        dynamicMapRenderMode = mode
        EventBus.publish(DynamicMapRenderModeEvent(dynamicMapRenderMode))
        save()
    }
    ...
    private fun centeredGridCalibration(bundle: DynamicMapBundle): GridCalibration {
        val cellPx = gridCalibration.effectiveCellSizeInPixels()
        return gridCalibration.copy(
            offsetX = -bundle.cols * cellPx / 2.0,
            offsetY = -bundle.rows * cellPx / 2.0,
        )
    }
...
class MapFogOfWarService(private val halfFogCells: Int = DEFAULT_HALF_FOG_CELLS) {
    ...
    private var defaultCols: Int = halfFogCells * 2
    private var defaultRows: Int = halfFogCells * 2
    private var defaultColOffset: Int = -halfFogCells
    private var defaultRowOffset: Int = -halfFogCells
    ...
    fun ensureInitialized() {
        ...
        fogColOffset = defaultColOffset
        fogRowOffset = defaultRowOffset
        EventBus.publish(
            FogOfWarSetupEvent(
                cols = cols,
                rows = rows,
    ...
    fun configureCoverage(cols: Int, rows: Int, colOffset: Int = 0, rowOffset: Int = 0) {
        defaultCols = cols.coerceAtLeast(1)
        defaultRows = rows.coerceAtLeast(1)
        defaultColOffset = colOffset
        defaultRowOffset = rowOffset

        val current = fogState
        if (
            current != null &&
            current.cols == defaultCols &&
            current.rows == defaultRows &&
            fogColOffset == defaultColOffset &&
            fogRowOffset == defaultRowOffset
        ) {
            replayState()
        ...
        fogState = FogOfWarState(cols = defaultCols, rows = defaultRows)
        fogColOffset = defaultColOffset
        fogRowOffset = defaultRowOffset
        replayState()
    }
